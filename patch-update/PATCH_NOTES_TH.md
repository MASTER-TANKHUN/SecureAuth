...
## 🚀 Post-Audit Remediation III (29-04-2026 - Gemini 3.1 Pro Audit)

### 🔴 HIGH Priority Fixes

#### 1. Persistent Account Lockout DoS
- **ไฟล์:** `server/routes/auth.js` (Login handler)
- **ปัญหา:** เมื่อบัญชีหมดระยะเวลาล็อค ระบบข้ามการตรวจสอบโดยไม่ล้างค่า `failed_login_attempts` ทำให้เมื่อใส่รหัสผ่านผิดซ้ำ บัญชีจะถูกล็อคต่อทันทีแบบไม่รู้จบ
- **แก้ไข:** เพิ่มคำสั่ง `statements.resetFailedAttempts.run()` เพื่อล้างจำนวนครั้งที่เข้าสู่ระบบผิดเมื่อระยะเวลาล็อคสิ้นสุดลง

### 🟠 MEDIUM Priority Fixes

#### 2. ไม่ส่งอีเมลแจ้งเตือนเมื่อเปลี่ยนรหัสผ่านใน `/change-password`
- **ไฟล์:** `server/routes/auth.js`
- **ปัญหา:** ไม่มีการเรียกใช้ `sendPasswordChangeAlertEmail` หลังจากการเปลี่ยนรหัสผ่านสำเร็จ ทำให้ผู้ใช้ไม่ได้รับการแจ้งเตือน
- **แก้ไข:** เพิ่มการเรียกใช้ฟังก์ชันส่งอีเมลแจ้งเตือนทันทีที่มีการอัปเดตรหัสผ่านสำเร็จ

### 🟡 LOW Priority Fixes

#### 3. ลำดับ Middleware ใน `/mfa/setup` ไม่ถูกต้อง
- **ไฟล์:** `server/routes/auth.js`
- **ปัญหา:** นำ `mfaAttemptLimiter` มาไว้หน้า `authenticate` ทำให้คำขอที่ไม่มีสิทธิ์หรือไม่มี Token ก็สามารถเข้ามากินโควต้า Rate Limit ได้
- **แก้ไข:** สลับนำ `authenticate` ขึ้นก่อนเพื่อกรองคำขอที่ไม่ถูกต้องทิ้งไปก่อนถึง Rate Limiter

#### 4. Dummy Hash ความยาวไม่ตรงและไม่ Sync กับ Config
- **ไฟล์:** `server/routes/auth.js`
- **ปัญหา:** ค่า Dummy Argon2 Hash ที่ใช้สำหรับป้องกัน Timing Attack มีขนาดไม่ตรงกับมาตรฐาน (32 bytes) และไม่ตอบสนองต่อการเปลี่ยน Config
- **แก้ไข:** เพิ่มระบบสร้าง Dummy Hash จากโค้ดจริงตอนเริ่มต้น `getDummyHash()` และทำ Caching ไว้เพื่อให้มีความยาวเป๊ะ 100% และซิงค์กับ Config (timeCost/memoryCost) ปัจจุบันเสมอ

---

## 🚀 Post-Audit Remediation IV (29-04-2026 - Antigravity Audit)

### 🔴 แก้ไขระดับ HIGH

#### 1. ระบบตรวจจับการขโมย Refresh Token (Token Family Revocation)
- **ไฟล์:** `server/routes/auth.js`, `server/models/db.js`
- **ปัญหา:** ระบบเดิมไม่สามารถตรวจจับการ "Replay" ของ Refresh Token ที่ถูกขโมยได้ เมื่อแฮกเกอร์ใช้โทเค็นที่เหยื่อเคยใช้ไปแล้ว ระบบแค่แจ้งว่าโทเค็นใช้ไม่ได้ แต่ไม่ได้ระงับโทเค็นใบใหม่ที่แฮกเกอร์ถืออยู่
- **แก้ไข:** เพิ่มการค้นหา Token แบบไม่จำกัดสถานะ หากพบว่ามีการใช้ Token ที่ถูก Revoke ไปแล้ว ระบบจะสั่งระงับ Refresh Token **ทุกใบ** ของผู้ใช้คนนั้นทันที และเพิ่ม `session_version` เพื่อเตะ Access Token ทุกใบออก

#### 2. ช่องโหว่ DoS จากระบบ Account Lockout ต่อผู้ใช้ที่ล็อกอินอยู่
- **ไฟล์:** `server/middleware/auth.js`
- **ปัญหา:** Middleware `authenticate` มีการเช็ก `locked_until` ทุกครั้ง ทำให้แฮกเกอร์ที่รู้แค่อีเมลเหยื่อสามารถสแปมล็อกอินให้บัญชีล็อค ซึ่งจะส่งผลให้เจ้าของบัญชีที่ล็อกอินทำงานอยู่ถูกเตะออกจากระบบทันที
- **แก้ไข:** นำการเช็ก `locked_until` ออกจาก Middleware เพื่อให้การ Lockout ส่งผลเฉพาะการ "ล็อกอินใหม่" เท่านั้น ไม่กระทบกับ Session เดิมที่ยังไม่หมดอายุ

### 🟠 แก้ไขระดับ MEDIUM

#### 3. Email Enumeration จากการหน่วงเวลาของระบบส่งอีเมล (SMTP Timing)
- **ไฟล์:** `server/routes/auth.js`
- **ปัญหา:** เซิร์ฟเวอร์มีการใช้ `await` รอผลการส่งอีเมลในหน้าสมัครสมาชิก, ลืมรหัสผ่าน และเปลี่ยนอีเมล ทำให้เกิดความต่างของเวลา (Latency) ที่สังเกตได้ว่าอีเมลนั้นมีอยู่ในระบบจริงหรือไม่
- **แก้ไข:** เปลี่ยนการส่งอีเมลเป็นแบบ Asynchronous โดยใช้ `.catch()` จัดการ Error แทน ทำให้ Server ตอบกลับได้ทันทีหลังจากการแฮชรหัสผ่าน/โทเค็นเสร็จสิ้น ปิดช่องทางจับเวลา

#### 4. ขาดการใช้ Database Transaction ในเส้นทางที่สำคัญ
- **ไฟล์:** `server/routes/auth.js`
- **ปัญหา:** ใน Endpoint `/change-password` และ `/delete-account` มีการอัปเดตข้อมูลหลายตารางและยกเลิก Session ต่อกันโดยไม่มี Transaction หากเกิดข้อผิดพลาดกลางคันอาจทำให้ข้อมูลในฐานข้อมูลไม่สอดคล้องกัน (Inconsistent State)
- **แก้ไข:** ครอบคำสั่งที่แก้ไขข้อมูลทั้งหมดไว้ใน `db.transaction()` เพื่อรับประกันความถูกต้องของข้อมูลแบบ Atomic

### 🟡 แก้ไขระดับ LOW

#### 5. ช่องโหว่ XSS ใน Email Templates
